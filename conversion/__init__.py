...
def get_model_class(name: str, mmproj: bool = False) -> Type[ModelBase]:
    ...
    if name in ModelBase._model_classes[model_type]:
    ...
    try:
        ...
        logger.warning(f"Could not import conversion module '{module_name}': {e}")

    if name in ModelBase._model_classes[model_type]:
        return ModelBase._model_classes[model_type][name]
    raise NotImplementedError(f"Architecture {name!r} failed to load model class.")
